<table id="alt_pagination" class="table table-striped table-bordered display" style="width:100%">
    ...
    <tbody>
        ...
        if (count($product_det) > 0) {
            $i = 1;
            foreach ($product_det as $b) {

                 if($b->home_page == 0){
                        $show = "<button class='btn btn-success btn-sm btn-grad' onclick='changestatus2(1,".$b->ppid.")'><i class='icon-ok'></i> Show In Home Page </button>";
                    } else {

                        $show="<button class='btn btn-danger btn-sm btn-grad' onclick='changestatus2(0,".$b->ppid.")'><i class='icon-remove'></i> Dont Show In Home Page </button>";
					...
					if($b->pstatus=='0')
		            {
			            ...
			            $chng="<button class='btn btn-danger btn-sm btn-grad' onclick='changestatus(1,".$b->ppid.")'><i class='icon-remove'></i> Deactivate </button>";

		            }
		            else
		            {
			            $status="<span class='btn btn-danger btn-sm btn-grad'>In-Active</span>";
			            $chng="<button class='btn btn-success btn-sm btn-grad' onclick='changestatus(0,".$b->ppid.")'><i class='icon-ok'></i> Activate </button>";
		            }
