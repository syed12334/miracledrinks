...
//*************Banner**************//
    ...
    function banner_save()    
    {
		...
		$uploaddir = '../assets/banner_images/';
		$uploadDBdir= "assets/banner_images/";
    	...
    	redirect('homepage/banner');
				...
				public function banner_status()
        		{
					...
					if($status == 2)
					{
						$this->master_db->deleterecord('banner',array("id"=>$items));
					}
					$this->data['banner'] = $this->load->view('getbanner', $this->data ,TRUE);
    ...
    function client_save()
    {
       ...
       $img1up = false;
		$arry = array("gif","jpg","png","ico","jpeg");
		$uploaddir = '../assets/client_images/';
		$uploadDBdir= "assets/client_images/";
...
//    ************************************************************Advertise****************************************************
    ...
    public function advertise()
    {
        ...
        else if($_SERVER['REQUEST_METHOD']=='POST')
        {
            ...
            $img1up = $img2up = false;
            $arry = array("gif","jpg","png","ico","jpeg");
            $uploaddir = '../assets/advertise/';
            $uploadDBdir= "assets/advertise/";
    ...
    public function gifts_status()
    {
       $items = $this->input->post('items');
		...
		if($status == 2)
		{
			$this->master_db->deleterecord('gifts',array("id"=>$items));
		}
		$this->data['gifts'] = $this->load->view('getgifts', $this->data ,TRUE);
    ...
    function gift_save()
    {
      ...
      /*  $this->load->library('upload', $config);
        if($this->upload->do_upload('imag'))
        {*/
			$pageurl = str_replace("-","_",$this->master_db->create_unique_slug(trim(preg_replace('!\s+!', ' ',$this->input->post('name'))),'gifts','page_url'));
            $db = array(
                'order_no'=>$this->input->post('orderno'),
            ...
            $res=$this->master_db->insertRecord('gifts',$db);


            $pros = $this->input->post('proname');
            ...
            if($pros){
                ...
                foreach($pros as $value){
                    ...
                    $this->master_db->insertRecord('gift_products',$db);

                }
            }
            
            $sql = "CREATE VIEW gift_".$pageurl."_view
					AS
    				...
    				cl.status=0 and p.status=0 and s.status=0 and (ps.selling_price!='' or CAST(ps.selling_price AS DECIMAL)>0) and ps.stock>0 and g.id='".$res."'
    				group by ps.id order by pi.order_no ";
            $this->master_db->runQuery($sql);

            $this->home_db->resizeImagef($file,91, 93);
        }
    ...
    public function gifts_edit()
    {
        ...
        else if($_SERVER['REQUEST_METHOD']=='POST')
        {
            ...
            $res=$this->master_db->updateRecord('gifts',$db,$id);


            $img1up = $img2up = false;
            $arry = array("gif","jpg","png","ico","jpeg");
            $uploaddir = '../assets/gift_images/';
            $uploadDBdir= "assets/gift_images/";
            ...
            $this->master_db->deleterecord('gift_products','gift_id',$id);
            $pros = $this->input->post('proname');
