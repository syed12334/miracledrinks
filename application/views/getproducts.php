    ...
    <tbody>
        ...
        if (count($product_det) > 0) {
            ...
            foreach ($product_det as $b) {
                 ...
                 if($b->home_page == 0){
                        $show = "<button class='btn btn-info btn-sm btn-grad' onclick='changestatus2(1,".$b->ppid.")'><i class='icon-ok'></i> Show In Bundle Product </button>";
                    } else {

                        $show="<button class='btn btn-primary btn-sm btn-grad' onclick='changestatus2(0,".$b->ppid.")'><i class='icon-remove'></i> Dont Show In Bundle Product </button>";
